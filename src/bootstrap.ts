...
if (existingEmployee?.authUserId) {
    ...
    const existingAccount = await context.internalAdapter.findUserByEmail(normalizedEmail, { includeAccounts: true });
    const user = existingAccount?.user ?? await context.internalAdapter.createUser({
    ...
    try {
        await Employee.findOneAndUpdate(
            { email: normalizedEmail },
            { name, email: normalizedEmail, role: "admin", avatar: name.charAt(0).toUpperCase(), authUserId: user.id },
