...
class SongService {
    public async create(data: ICreateSongDTO) {
        ...
        if (!isTabOpen(tab.status)) {
            throw new AppError("A comanda precisa estar aberta para adicionar musicas.", 409);
        }

        const lastSong = await Song.findOne({ status: "queued" }).sort({ position: -1 });
    ...
    public async cancel(id: string) {
        ...
        if (song.status !== "queued") {
            throw new AppError("A musica atual ou ja finalizada nao pode ser cancelada.", 409);
        }
    ...
    public async advance() {
        ...
        const next = await Song.findOne({ status: "queued" }).sort({ position: 1, requestedAt: 1 });
        if (next) {
            next.status = "playing";
            next.position = 0;
    ...
    private async reorganizeQueue(): Promise<void> {
        const queuedSongs = await Song.find({ status: "queued" }).sort({ position: 1, requestedAt: 1 });
        await Promise.all(
