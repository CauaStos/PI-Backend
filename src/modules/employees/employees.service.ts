...
class EmployeeService {
    public async create(data: ICreateEmployeeDTO) {
        ...
        if (!account?.user?.id) throw new AppError("Nao foi possivel criar a conta.", 500);
        try {
    ...
    public async delete(id: string, data: Pick<IUpdateEmployeeDTO, "headers" | "actorUserId">) {
        const employee = await Employee.findById(id);
        if (!employee) {
            throw new AppError("Funcionario nao encontrado.", 404);
        }
        ...
        if (employee.role === "admin" && await Employee.countDocuments({ role: "admin" }) <= 1) {
