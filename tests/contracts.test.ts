...
describe("schemas de dominio", () => {
  ...
  it("productSchema rejeita preco negativo e estoque negativo", () => {
    expect(
      productSchema.safeParse({ ...baseProduct, price: -1 }).success
    ).toBe(false)
    expect(
      productSchema.safeParse({ ...baseProduct, stock: -5 }).success
    ).toBe(false)
  })
  ...
  it("tabSchema aceita comanda aberta e rejeita status desconhecido", () => {
    ...
    expect(tabSchema.safeParse({ ...base, status: "fechada" }).success).toBe(
      false
    )
  })
})
